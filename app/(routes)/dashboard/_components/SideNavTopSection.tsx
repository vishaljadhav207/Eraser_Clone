...
function SideNavTopSection({ user,setActiveTeamInfo }: any) {
  ...
  const [activeTeam, setActiveTeam] = useState<TEAM>(); //for login team TeamName
  useEffect(() => {
    user && getTeamList(); //if user available then only it will getTeamList from user table from DB
  }, [user]);
  ...
  return ( 
    <div >
      <Popover >
        ...
        <PopoverContent className="ml-3 p-4 mr-3">
          {/* Team secton */}
          <div>
            {teamList?.map((team, index) => (
              <h2
                key={index}
                className={`p-2 hover:bg-blue-500 rounded-lg hover:text-white mb-1 cursor-pointer ${
          ...
          <div>
            ...
            <LogoutLink>
              <h2 className="flex gap-2 items-center p-2 hover:bg-slate-100 rounded-lg text-sm cursor-pointer">
